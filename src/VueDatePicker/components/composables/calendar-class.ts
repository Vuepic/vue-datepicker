...
export const useCalendarClass = (modelValue: WritableComputedRef<InternalModuleValue>) => {
    ...
    const setHoverDate = (day: UnwrapRef<ICalendarDay>): void => {
        if (!day.current && config.value.hideOffsetDates) {
            return;
        }
    ...
    const isActiveDate = (calendarDay: ICalendarDay): boolean => {
        ...
        if (config.value.hideOffsetDates && !calendarDay.current) return false;
        if (!config.value.range) {
            if (config.value.multiDates && Array.isArray(modelValue.value)) {
        ...
        if (config.value.modelAuto && Array.isArray(modelValue.value)) {
    ...
    const isSingleInModelAuto = (): boolean => {
        if (config.value.modelAuto && Array.isArray(internalModelValue.value)) {
            return !!internalModelValue.value[0];
        }
        return false;
    };

    const isModelAutoActive = () => {
        if (config.value.modelAuto) return isModelAuto(internalModelValue.value);
        return true;
    };

    // Check if date is hovered, for single picker mode
    const dateHover = (day: ICalendarDay) => {
        if ((Array.isArray(modelValue.value) && modelValue.value.length) || config.value.weekPicker) return false;
        const isInRange = config.value.range ? !rangeActiveStartEnd(day) && !rangeActiveStartEnd(day, false) : true;

        return (
            !isDisabled(day.value) && !isActiveDate(day) && !(!day.current && config.value.hideOffsetDates) && isInRange
        );
    };
    ...
    const isActive = (day: ICalendarDay) => {
        if (config.value.range) {
            return config.value.modelAuto ? isSingleInModelAuto() && isActiveDate(day) : false;
        }
    ...
    const highlighted = (day: ICalendarDay) => {
        if (config.value.highlight) {
            return matchDate(day.value, config.value.highlight);
        }
        return false;
    };
    ...
    const disableHighlight = (day: ICalendarDay) =>
        isDisabled(day.value) && config.value.highlightDisabledDays === false;

    // Check if the given week day should be highlighted
    const highlightedWeekDay = (day: ICalendarDay) => {
        return config.value.highlightWeekDays && config.value.highlightWeekDays.includes(day.value.getDay());
    };

    const isBetween = (day: ICalendarDay) => {
        if (
            (config.value.range || config.value.weekPicker) &&
            (config.value.multiCalendars > 0 ? day.current : true) &&
            !isDisabled(day.value) &&
            isModelAutoActive() &&
            !(!day.current && config.value.hideOffsetDates) &&
            !isActiveDate(day)
        ) {
    ...
    const sharedClasses = (day: ICalendarDay): Record<string, boolean> => {
        return {
            ...
            dp__pointer: !config.value.disabled && !(!day.current && config.value.hideOffsetDates),
            dp__cell_disabled: isDisabled(day.value),
            [config.value.dayClass ? config.value.dayClass(day.value) : '']: true,
            dp__cell_highlight:
            ...
            [config.value.calendarCellClassName]: !!config.value.calendarCellClassName,
            dp__today: !config.value.noToday && isDateEqual(day.value, today.value) && day.current,
        };
    };
    ...
    const weekPickerClasses = (day: ICalendarDay): Record<string, boolean> => {
        return {
            ...
            dp__range_between_week: isBetween(day) && config.value.weekPicker,
        };
    };
    ...
    const rangeDateClasses = (day: ICalendarDay): Record<string, boolean> => {
        return {
            ...
            dp__range_between: isBetween(day) && !config.value.weekPicker,
    ...
    const getModeClasses = (day: ICalendarDay) => {
        if (config.value.range) {
            if (config.value.autoRange) return autoRangeClasses(day);
            return rangeDateClasses(day);
        }
        if (config.value.weekPicker) {
