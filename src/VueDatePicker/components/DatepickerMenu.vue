<template>
    <transition appear :name="transitions.menuAppear" mode="out-in" :css="!!transitions">
        <div :id="uid ? `dp-menu-${uid}` : undefined" tabindex="0" ref="dpMenuRef" role="dialog"
            :aria-label="ariaLabels.menu" :class="dpMenuClass" @mouseleave="clearHoverDate" @click="handleDpMenuClick"
            @keydown.esc="handleEsc" @keydown.space="handleSpace" @keydown.left.prevent="handleArrowKey('left')"
            @keydown.up.prevent="handleArrowKey('up')" @keydown.down.prevent="handleArrowKey('down')"
            @keydown.right.prevent="handleArrowKey('right')" @keydown="checkShiftKey">
            ...
            <div :class="presetRanges.length ? 'dp__menu_content_wrapper' : null">
                <div class="dp__preset_ranges" v-if="presetRanges.length">
                    <div v-for="(preset, i) in presetRanges" :key="i" :style="preset.style || {}"
                        class="dp__preset_range" @click="presetDateRange(preset.range)">
                        {{ preset.label }}
                    </div>
                </div>
                <div class="dp__instance_calendar" ref="calendarWrapperRef" role="document">
                    <div :class="menuCalendarClassWrapper">
                        <div v-for="(instance, i) in calendarAmm" :key="instance" :class="calendarInstanceClassWrapper">
                            <component :is="monthYearComponent ? monthYearComponent : MonthYearPicker" :ref="
                                (el) => {
                                    if (el) monthYearPickerRefs[i] = el;
                                }
                            " v-if="!disableMonthYearSelect && !timePicker" v-bind="{
    months,
    years,
    filters,
    monthPicker,
    ...
    yearPicker,
                                ...
                                @month-year-select="monthYearSelect" @overlay-closed="focusMenu">
                            ...
                            " v-bind="calendarProps" v-model:flow-step="flowStep" :instance="instance"
                    ...
                    <div>
                        <component v-if="enableTimePicker && !monthPicker && !weekPicker"
                            :is="timePickerComponent ? timePickerComponent : TimePickerCmp" ref="timePickerRef" v-bind="{
                                is24,
                                hoursIncrement,
                                minutesIncrement,
                                hoursGridIncrement,
                ...
                <div class="dp__now_wrap" v-if="showNowButton">
                    ...
                    <button v-if="!$slots['now-button']" type="button" role="button" class="dp__now_button"
                        @click="selectCurrentDate">
                        {{ nowButtonLabel }}
                    </button>
                </div>
            </div>
            <component v-if="!autoApply || keepActionRow" :is="actionRowComponent ? actionRowComponent : ActionRow"
                v-bind="{
                    calendarWidth,
                    selectText,
                    cancelText,
                    ...
                    modelAuto,
                    partialRange,
                }" @close-picker="$emit('closePicker')" @select-date="$emit('selectDate')"
...
import type {
AreaLabels, CalendarRef,
DynamicClass,
ICalendarDate,
IDefaultSelect,
InternalModuleValue, ITransition, MenuChildCmp,
MonthYearPickerRef,
TimePickerRef,
WeekStartNum
} from '@/interfaces';
    ...
    return dates.value(instance).map((date) => {
        return {
            ...
            days: date.days.map((calendarDay) => {
                ...
                const highlightedWeekDay = props.highlightWeekDays && props.highlightWeekDays.includes(calendarDay.value.getDay());
                const isBetween =
