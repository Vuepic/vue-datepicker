<template>
  <div v-if="!rootProps.disabled" class="dp--time-input">
    <div
      ...
      <template v-else>
        <button
          ...
          :class="{
            ...
            'dp--inc-dec-button-disabled': disabledArrowUpBtn(timeInput.type),
        ...
        >
          ...
          <template v-else>
            <slot name="tp-inline-arrow-up">
              <span class="dp--tp-inline-btn-bar dp--tp-btn-in-l"></span>
              <span class="dp--tp-inline-btn-bar dp--tp-btn-in-r"></span>
            </slot>
          </template>
        </button>
        <button
        ...
        <button
          ...
          :class="{
            ...
            'dp--inc-dec-button-disabled': disabledArrowDownBtn(timeInput.type),
            'dp--hidden-el': timeOverlayOpen,
          }"
    ...
    <template v-for="(timeInput, i) in timeInputOverlays" :key="i">
      <transition :name="transitionName(overlays[timeInput.type])" :css="showTransition">
        <SelectionOverlay
          ...
          :level="timeConfig.timePickerInline || rootProps.timePicker ? 1 : 2"
        ...
        >
          <template #button-icon>
            <slot name="clock-icon">
              <component :is="timeConfig.timePickerInline ? CalendarIcon : ClockIcon" v-if="!$slots['clock-icon']" />
            </slot>
          </template>
          ...
          <template v-if="$slots[`${timeInput.type}-overlay-header`]" #header>
            <slot :name="`${timeInput.type}-overlay-header`" :toggle="() => toggleOverlay(timeInput.type)" />
          </template>
        </SelectionOverlay>
      </transition>
    </template>
...
<script lang="ts" setup>
  ...
  import { add, getHours, getMinutes, getSeconds, isAfter, isBefore, isEqual, set, sub, type Duration } from 'date-fns';
  ...
  import { useTransitions, useContext, useHelperFns, useDateUtils, useUtils } from '@/composables';
  import type {
    DynamicClass,
    Hour12,
    InvalidTimesConfig,
  ...
  const timeColClass = computed(
    (): DynamicClass => ({
      'dp--time-col': true,
      'dp--time-col-block': !timeConfig.value.timePickerInline,
      'dp--time-col-reg-block':
        !timeConfig.value.enableSeconds && timeConfig.value.is24 && !timeConfig.value.timePickerInline,
      'dp--time-col-reg-inline':
        !timeConfig.value.enableSeconds && timeConfig.value.is24 && timeConfig.value.timePickerInline,
      'dp--time-col-reg-with-button': !timeConfig.value.enableSeconds && !timeConfig.value.is24,
      'dp--time-col-sec': timeConfig.value.enableSeconds && timeConfig.value.is24,
      'dp--time-col-sec-with-button': timeConfig.value.enableSeconds && !timeConfig.value.is24,
    }),
  );
  ...
  const handleTimeValue = (type: TimeKey, inc = true, opts?: { keyboard?: boolean }): void => {
    ...
    if (isInRange) {
      emit(
        ...
        getTimeGetter(type)(addOrSub({ [type]: +props[type] }, { [type]: +timeConfig.value[`${type}Increment`] })),
      );
    }
