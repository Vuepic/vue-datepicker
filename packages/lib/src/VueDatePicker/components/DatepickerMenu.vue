<template>
  ...
  >
    ...
    >
      ...
      >
        <template v-for="(preset, i) in rootProps.presetDates" :key="i">
          <template v-if="preset.slot">
            <slot :name="preset.slot as never" :preset-date="presetDate" :label="preset.label" :value="preset.value" />
          </template>
          <template v-else>
            <button
              type="button"
      ...
      <div class="dp--instance-calendar">
        <component
          :is="displayComponent"
          ref="dyn-cmp"
          :collapse="collapse"
      ...
      <div v-if="$slots['right-sidebar']" class="dp--sidebar-right">
        <slot name="right-sidebar" v-bind="getSidebarProps" />
      </div>
    </div>
    <div v-if="$slots['action-extra']" class="dp--action-extra">
...
<script lang="ts" setup>
  ...
  const dpMenuClass = computed(
    (): DynamicClass => ({
      'dp--menu': true,
      'dp--menu-index': !inline.value.enabled,
      'dp--relative': inline.value.enabled,
      ...ui.value.menu,
    }),
  );
  ...
  const onKeyDown = (ev: KeyboardEvent) => {
    ...
    if (ev.key === EventKey.home || ev.key === EventKey.end) {
      return callChildFn('selectWeekDate', ev.key === EventKey.home, (ev.target as HTMLElement).getAttribute('id'));
    }
