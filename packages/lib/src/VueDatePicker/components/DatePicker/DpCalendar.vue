<template>
  <div :class="calendarParentClass">
    <div ref="calendar-wrap" :class="calendarWrapClass" role="grid">
      ...
      <div class="dp--calendar-header-separator"></div>
      <transition :name="transitionName" :css="!!transitions">
        <div v-if="showCalendar" class="dp--calendar" role="rowgroup" @mouseleave="isMouseDown = false">
          <div v-for="(week, weekInd) in calendarWeeks" :key="weekInd" class="dp--calendar-row" role="row">
            <div v-if="weekNumbers" class="dp--calendar-item dp--week-num" role="gridcell">
            ...
            >
              <div class="dp--cell-inner" :class="dayVal.classData">
                <slot v-if="$slots.day && showDay(dayVal)" name="day" :day="+dayVal.text" :date="dayVal.value"></slot>
                <template v-if="!$slots.day"> {{ dayVal.text }} </template>
                <template v-if="dayVal.marker && showDay(dayVal)">
                  <slot name="marker" :marker="dayVal.marker" :day="+dayVal.text" :date="dayVal.value">
                    <div
                ...
                >
                  <div v-if="dayVal.marker?.tooltip" class="dp--tooltip-content" @click="onTpClick">
                    <div v-for="(tooltip, i) in dayVal.marker.tooltip" :key="i" class="dp--tooltip-text">
                    ...
                    <div class="dp--arrow-bottom-tp" :style="tpArrowStyle"></div>
                  </div>
                </div>
              </div>
            </div>
...
<script lang="ts" setup>
  ...
  const triggerTransition = (month: number, year: number): void => {
    if (rootProps.transitions) {
      ...
      nextTick().then(() => (showCalendar.value = true));
    }
  };
  ...
  const markerClass = computed((): ((marker: Marker) => DynamicClass) => (marker) => {
    ...
    return {
      'dp--marker-base': true,
      'dp--marker-dot': defaultMarker.type === 'dot',
      'dp--marker-line': defaultMarker.type === 'line',
    };
  });
