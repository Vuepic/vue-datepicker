...
import type { CalendarDay, InternalModelValue } from '@/types';
...
export const useCalendarClass = () => {
  ...
  // Check if the date is hovered, for single picker mode
  const dateHover = (day: CalendarDay) => {
    ...
    return !isDisabled(day.value) && !isActiveDate(day) && !(!day.current && rootProps.hideOffsetDates) && isInRange;
  };
  ...
  const sharedClasses = (day: CalendarDay): Record<string, boolean> => {
    ...
    return {
      'dp--cell-offset': !day.current,
      'dp--pointer':
        !rootProps.disabled &&
        !(!day.current && rootProps.hideOffsetDates) &&
        !isDisabled(day.value) &&
        !minMaxRangeDate(day),
      'dp--cell-disabled': isDisabled(day.value) || minMaxRangeDate(day),
      'dp--cell-highlight':
      ...
      'dp--today': !rootProps.noToday && isDateEqual(day.value, today) && day.current,
  ...
  const isFixedRange = (value: InternalModelValue): value is [Date, Date] => {
    return (
      range.value.enabled &&
  ...
  const isFixedRangeExcludeActive = (day: CalendarDay, start: boolean, isRangeStart: boolean, isRangeEnd: boolean) => {
    if (!isFixedRange(modelValue.value) || !hoveredDate.value) return false;
    if (start) {
      return (
        range.value.fixedEnd &&
        isDateEqual(day.value, hoveredDate.value) &&
        isBefore(day.value, modelValue.value[0]) &&
        !isRangeStart
      );
    }
    return (
      ...
      isDateEqual(day.value, hoveredDate.value) &&
      isAfter(day.value, modelValue.value[1]) &&
      !isRangeEnd
    );
  };

  const isExtendedFixedStartEnd = (day: CalendarDay, start: boolean) => {
    if (!isFixedRange(modelValue.value) || !hoveredDate.value) return false;
    if (start) {
      return range.value.fixedEnd && isAfter(day.value, hoveredDate.value) && isBefore(day.value, modelValue.value[0]);
    }
    return range.value.fixedStart && isBefore(day.value, hoveredDate.value) && isAfter(day.value, modelValue.value[1]);
  };

  // Get a set of classes for the range
  ...
  // Get a set of classes for auto range
  ...
  // Return a specific set of classes depending on the config, since we don't need to check for all
