...
export const useExternalInternalMapper = () => {
  ...
  const mapMonthExternalToInternal = (value: MonthModel | MonthModel[]): Date | Date[] => {
    ...
    if (Array.isArray(value)) {
      if (multiDates.value.enabled) {
        return value.map((val) => convertCustomModeType(val, set(today, { month: +val.month, year: +val.year })));
      }
      return checkRangeEnabled(
        () => [
  ...
  const toModelType = (val: Date): string | number | Date | null => {
    ...
    if (rootProps.modelType) {
      ...
      if (rootProps.modelType === 'format' && typeof formats.value.input === 'string') return formatSelectedDate(val);
