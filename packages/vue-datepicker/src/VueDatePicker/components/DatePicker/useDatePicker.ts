...
export const useDatePicker = (
  ...
  const handleNextCalendarAutoRange = (date: string | Date) => {
    ...
    if (multiCalendars.value.count > 0) {
      for (let i = 1; i < multiCalendars.value.count; i++) {
        const next = getNextMonthYear(set(getDate(date), { year: year.value(i - 1), month: month.value(i - 1) }));
  ...
  const setFixedDateRange = (day: CalendarDay) => {
    if (includesDisabled(day.value) || !checkMinMaxRange(day.value, modelValue.value, range.value.fixedStart ? 0 : 1)) {
